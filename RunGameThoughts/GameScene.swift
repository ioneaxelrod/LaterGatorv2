...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func addObject() {
        ...
        if obstacle.sprite.name == ImageNameConstants.TRASH_CAN_SPRITE_NAME ||
                   ...
                   obstacle.sprite.name == ImageNameConstants.SPIDER_SPRITE_NAME {
 ...
 */
    func updateLabel() {
        labelNode.text = "Monster Points: \(theMonster.monsterScore) \n Player Points: \(thePlayer.score)"
    }
 ...
 */
    func gameOver(win: Bool) {
        run(
                SKAction.run() {
                    self.view?.presentScene(GameOverScene(size: self.size, points: self.thePlayer.score))
                }
        )
    }
    ...
    func objectCollidedWithPlayer(object: SKSpriteNode, player: SKSpriteNode) {
        ...
        case ImageNameConstants.SPIDER_SPRITE_NAME:
            thePlayer.decreaseScore(points: GameBalanceConstants.MEDIUM_HIT)
            theMonster.moveCloser(number: GameBalanceConstants.MEDIUM_HIT)
